...
describe("Cloudinary and Shopify environment contracts", () => {
  it("documents Cloudinary names without real values", () => {
    ...
    expect(template).toContain("VITE_CLOUDINARY_CLOUD_NAME=");
    expect(template).toContain("VITE_CLOUDINARY_BANNER_VIDEO_PUBLIC_ID=");
    expect(template).toContain("VITE_CLOUDINARY_BANNER_POSTER_PUBLIC_ID=");
    expect(template).not.toMatch(/cloudinary:\/\/[^\n]+/i);
    const responsive = readFileSync(resolve(process.cwd(), "RESPONSIVE_VERIFICATION.md"), "utf8");
    expect(responsive).toContain("VITE_CLOUDINARY_BANNER_VIDEO_PUBLIC_ID");
    expect(responsive).toContain("VITE_CLOUDINARY_BANNER_POSTER_PUBLIC_ID");
